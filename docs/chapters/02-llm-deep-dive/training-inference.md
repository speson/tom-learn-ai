...
```
┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│  1. 사전학습  │ →  │  2. 파인튜닝  │ →  │  3. RLHF    │
│ Pre-training │    │ Fine-tuning  │    │ 인간 피드백  │
│              │    │              │    │ 강화학습     │
│ 인터넷 텍스트 │    │ 지시-응답    │    │ 선호도 학습  │
│ 다음 토큰 예측│    │ 쌍 데이터    │    │             │
└─────────────┘    └─────────────┘    └─────────────┘
  "언어를 배움"      "지시를 따르게"     "좋은 답변을 하게"
```
...
Step 1: "AI란" → 다음 토큰 확률 계산
        "무엇" (0.35), "인공" (0.25), "기계" (0.15), ...
        → "무엇" 선택

Step 2: "AI란 무엇" → 다음 토큰 확률 계산  
        "인가" (0.40), "일까" (0.30), "이며" (0.10), ...
        → "인가" 선택
...
```
[학습]
인터넷 텍스트 → 사전학습(다음토큰예측) → 파인튜닝(지시따르기) → RLHF(품질향상)
                                                                    │
                                                              학습된 모델
                                                                    │
[추론]                                                              ▼
사용자 입력 → 토큰화 → 임베딩 → 트랜스포머 × N → 확률 분포 → 토큰 선택 → 디토큰화 → 출력
```
